...
static short bufA_i[buflen], bufA_q[buflen];
static short bufB_i[buflen], bufB_q[buflen];

int masterInitialised = 0;
int slaveUninitialised = 0;
int Device_Streaming = 0;

unsigned int tunner_mode = 1;
...
void Print_UpdateMSG(sdrplay_api_ErrT err)
{
	if (err == 1)
	{
		mexPrintf("Command Failed...\n");
	}
	else if (err == 2)
	{
		mexPrintf("NULL pointer or invalid operating mode...\n");
	}
	else if (err == 3)
	{
		mexPrintf("One or more parameters are set incorrectly...\n");
	}
	else if (err == 7)
	{
		mexPrintf("HW error occured during tuner initialisation...\n");
	}
	else if (err == 8)
	{
		mexPrintf("Failed to update sample rate...\n");
	}
	else if (err == 9)
	{
		mexPrintf("Failed to update Rf frequency...\n");
	}
	else if (err == 10)
	{
		mexPrintf("Failed to update gain...\n");
	}
	else if (err == 11)
	{
		mexPrintf("Feature not enabled...\n");
	}
	else if (err == 12)
	{
		mexPrintf("Communication channel with service broken...\n");
	}
	else
	{
		mexPrintf("Successful completion!\n");
	}
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs > 0)
	{
		...
		else if (strcmp("update_dualtuner", cmd) == 0)
		{
			tunner_mode = mxGetScalar(prhs[1]);
			if (tunner_mode == 1)	{
					chosenDevice.tuner = sdrplay_api_Tuner_Both;
					chosenDevice.rspDuoMode = sdrplay_api_RspDuoMode_Dual_Tuner;
				}
			else if (tunner_mode	== 0)	{
					chosenDevice.rspDuoMode = sdrplay_api_RspDuoMode_Single_Tuner;
				}

			if (Device_Streaming == 1)	{
				mexPrintf("Please close and re-start the device. Updating while streaming is not currently supported.\n");
			...
			if (Device_Streaming == 0)
			{
				...
				if ((err = sdrplay_api_Init(chosenDevice.dev, &cbFns, NULL)) != sdrplay_api_Success)
				{
					...
					if (err == sdrplay_api_StartPending) // This can happen if we're starting in master / slave mode as a slave and the master is not yet running
					{
						...
						{
#ifdef _WIN32
							Sleep(1000); // Sleep takes milliseconds in Windows
#else
							sleep(1); // sleep takes seconds in POSIX
#endif
